...
class LLMTreeReductionV3(Scene):
    def construct(self):
        ...
        # Title
        title = Text("Distributed Binder Design Query", font="Arial", font_size=32, color=COLORS["text"])
        ...
        self.play(
            ...
            FadeIn(legend),
            run_time=0.86  # 44% faster
        )

        # Show "dispatching" indicator
        dispatch_text = Text("Dispatching to 8 Aurora nodes...", font="Arial", font_size=22, color=COLORS["arrow"])
        dispatch_text.next_to(leaf_query_box, DOWN, buff=0.3)
        self.play(FadeIn(dispatch_text))

        # Pulse nodes as processing
        self.play(
            *[node[0].animate.set_fill(COLORS["node_active"], opacity=0.7) for node in leaf_nodes],
            run_time=0.29  # 44% faster
        )
        self.wait(0.29)  # 44% faster
        ...
        results_label = Text("Results streaming back...", font="Arial", font_size=22, color=COLORS["node_complete"])
        results_label.move_to(UP * 1.5)
        self.play(FadeIn(results_label))
        ...
        combine2_label = Text("Second reduction...", font="Arial", font_size=22, color=COLORS["combine"])
        combine2_label.move_to(UP * 2.5)
        self.play(FadeIn(combine2_label))
        ...
        final_label = Text("Final synthesis...", font="Arial", font_size=22, color=COLORS["winner"])
        final_label.to_edge(UP, buff=0.5)
        self.play(FadeOut(title), FadeIn(final_label))
        ...
        # Stats at bottom
        #stats = VGroup(
            #Text("8 parallel queries → 7 reductions → 1 winner", font="Arial", font_size=20, color=COLORS["subtext"]),
            #Text("Total time: 8.9s  |  Speedup: 3.6×", font="Arial", font_size=20, color=COLORS["node_complete"]),
        #).arrange(DOWN, buff=0.15)
        #stats.to_edge(DOWN, buff=0.3)

        #self.play(FadeIn(stats))
        self.wait(1.15)  # 44% faster
    ...
    def create_leaf_node_with_query(self, query):
        ...
        protein = Text(query["protein"], font="Arial", font_size=16, color=COLORS["text"], weight=BOLD)
        target = Text(f"↔ {query['target']}", font="Arial", font_size=14, color=COLORS["text"])
    ...
    def create_result_text(self, kd, modality):
        kd_text = Text(kd, font="Arial", font_size=14, color=COLORS["node_complete"], weight=BOLD)
        mod_text = Text(modality, font="Arial", font_size=14, color=COLORS["text"])
    ...
    def create_combine_result(self, name, kd):
        name_text = Text(name, font="Arial", font_size=16, color=COLORS["text"], weight=BOLD)
        kd_text = Text(kd, font="Arial", font_size=14, color=COLORS["node_complete"])
        return VGroup(name_text, kd_text).arrange(DOWN, buff=0.05)

    def create_query_box(self, title, query_text, color):
        box = RoundedRectangle(
            width=6.4, height=1.4, corner_radius=0.15,  # width reduced to 80% (was 8)
            color=color, stroke_width=2
        )
        box.set_fill(color, opacity=0.15)

        title_text = Text(title, font="Arial", font_size=20, color=color, weight=BOLD)
        query = Text(query_text, font="Arial", font_size=20, color=COLORS["text"])
    ...
    def create_legend(self):
        ...
        protein = Text("PARP16", font="Arial", font_size=16, color=COLORS["text"], weight=BOLD)
        target = Text("↔ NMNAT2", font="Arial", font_size=14, color=COLORS["text"])
        kd = Text("2.3 nM", font="Arial", font_size=15, color=COLORS["node_complete"], weight=BOLD)
        modality = Text("Monobody", font="Arial", font_size=14, color=COLORS["text"])
        ...
        label_protein = Text("← protein", font="Arial", font_size=14, color=COLORS["subtext"])
        label_target = Text("← target", font="Arial", font_size=14, color=COLORS["subtext"])
        label_kd = Text("← Kd (result)", font="Arial", font_size=14, color=COLORS["subtext"])
        label_modality = Text("← modality (result)", font="Arial", font_size=14, color=COLORS["subtext"])
        ...
        # Title
        title = Text("Leaf Node:", font="Arial", font_size=16, color=COLORS["text"], weight=BOLD)
    ...
    def create_winner_node(self):
        ...
        title = Text("★ WINNER ★", font="Arial", font_size=18, color=COLORS["winner"], weight=BOLD)
        name = Text("IL-6 Bispecific Nanobody", font="Arial", font_size=18, color=COLORS["text"], weight=BOLD)
        kd = Text("Kd = 50 pM", font="Arial", font_size=16, color=COLORS["node_complete"])
